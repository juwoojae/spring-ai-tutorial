...
class InMemoryDocumentVectorStore(
    ...
    fun addDocument(id: String, fileText: String, metadata: Map<String, Any>) {
        ...
        try {
            ...
            val document = Document(fileText, metadata + mapOf("id" to id)) //Document 는 Spring AI 가 제공해주는 컴포넌트
            val textSplitter = TokenTextSplitter.builder()
                ...
                .build()
            val chunks = textSplitter.split(document) // List<Document> 로 추가.
    ...
    fun addDocumentFile(id: String, file: File, metadata: Map<String, Any>) {
        ...
        try {
            ...
            addDocument(id, fileText, metadata) // documentId, 추출한 텍스트, 메티데이터로 document 를 vectorStore 에 추가
        } catch (e: Exception) {
    ...
    fun similaritySearch(query: String, maxResults: Int): List<DocumentSearchResultDto> {
        ...
        try {
            ...
            // 유사성 검색 실행
            val results = vectorStore.similaritySearch(request) ?: emptyList() //VectorStore 내부의 similaritSearch 메서드
